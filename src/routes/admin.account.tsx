...
const walletSchema = z.object({
  label: z.string().trim().min(2, "Enter a wallet label").max(60, "Keep the label under 60 characters"),
  ...
  address: z.string().trim().min(12, "Enter a valid wallet address").max(120, "Wallet address is too long"),
            ...
            <DialogContent className="border border-[var(--a-border-hi)] bg-[var(--a-bg-2)] text-[var(--a-fg)] sm:max-w-[560px]">
              <DialogHeader>
                ...
                <DialogDescription className="text-sm text-[var(--a-muted)]">
                  Store a treasury or settlement wallet for internal admin review. This stays in mock state for now.
                </DialogDescription>
              </DialogHeader>
              ...
              <form className="space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Wallet label" error={form.formState.errors.label?.message}>
                    <Input {...form.register("label")} className="border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="Treasury settlement wallet" />
                  </Field>
                  <Field label="Asset" error={form.formState.errors.asset?.message}>
                    <Input {...form.register("asset")} className="border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="USDT" />
                  </Field>
                  <Field label="Network" error={form.formState.errors.network?.message}>
                    <Input {...form.register("network")} className="border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="Ethereum" />
                  </Field>
                  <Field label="Provider" error={form.formState.errors.provider?.message}>
                    <Input {...form.register("provider")} className="border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="Fireblocks" />
                  </Field>
                </div>

                <Field label="Wallet address" error={form.formState.errors.address?.message}>
                  <Input {...form.register("address")} className="border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="0x... or bc1..." />
                </Field>

                <Field label="Memo / notes" error={form.formState.errors.memo?.message}>
                  <Textarea {...form.register("memo")} className="min-h-[92px] border-[var(--a-border)] bg-[var(--a-input)] text-[var(--a-fg)] placeholder:text-[var(--a-faint)] focus-visible:ring-[var(--a-accent)]" placeholder="Add operational notes, custody notes, or routing memo" />
                </Field>

                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--a-muted)]">Verification file</Label>
                  <label className="flex cursor-pointer items-center justify-between gap-3 rounded-md border border-dashed border-[var(--a-border-hi)] bg-[var(--a-surface)] px-4 py-3">
                    <div>
                      <p className="text-sm font-semibold text-[var(--a-fg)]">Upload proof of ownership</p>
                      <p className="text-xs text-[var(--a-muted)]">PDF, PNG or JPG evidence document</p>
                    </div>
                  ...
                  {selectedFile && <p className="a-mono text-[10px] text-[var(--a-accent)]">Attached: {selectedFile.name}</p>}
                </div>
      ...
      <div className="grid gap-4 lg:grid-cols-12">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="font-display text-2xl font-extrabold tracking-tight text-[var(--a-fg)]">{adminProfile.name}</h2>
                ...
                <div className="mt-2 grid gap-2 text-xs text-[var(--a-muted)] sm:grid-cols-2">
                  ...
                  <MetaLine icon={ShieldCheck} text={`Last login ${fmtDateTime(adminProfile.lastLogin)}`} />
                  <MetaLine icon={KeyRound} text={`Password rotated ${fmtDateTime(adminProfile.lastPasswordReset)}`} />
                </div>
        ...
        <BentoCard className="lg:col-span-4" eyebrow="Security" title="Access posture">
          <ul className="space-y-3 text-xs">
            <SecurityRow icon={ShieldCheck} label="Two-factor" value={adminProfile.twoFactor ? "Enabled" : "Disabled"} good={adminProfile.twoFactor} />
            <SecurityRow icon={CheckCircle2} label="Trusted devices" value={`${adminProfile.trustedDevices} devices`} />
            <SecurityRow icon={WalletCards} label="Linked wallets" value={`${wallets.length} records`} />
            <SecurityRow icon={UserCog} label="Ops teams" value={`${adminProfile.teams.length} scopes`} />
          </ul>
        </BentoCard>
        ...
        <BentoCard className="lg:col-span-5" eyebrow="Scope" title="Assigned teams">
          <ul className="space-y-2.5">
            {adminProfile.teams.map((team) => (
              <li key={team} className="rounded-md border border-[var(--a-border)] bg-[var(--a-bg-2)] px-3 py-2 text-sm font-semibold text-[var(--a-fg)]">
                {team}
              </li>
            ))}
          </ul>
        ...
        <BentoCard className="lg:col-span-7" eyebrow="Authority" title="Granted permissions">
          <div className="grid gap-2 sm:grid-cols-2">
            {adminProfile.permissions.map((permission) => (
              <div key={permission} className="flex items-center gap-2 rounded-md border border-[var(--a-border)] bg-[var(--a-bg-2)] px-3 py-2 text-sm text-[var(--a-fg-2)]">
        ...
        <BentoCard
          ...
          action={<span className="text-[10px] font-bold uppercase tracking-wider text-[var(--a-muted)]">{totals.activeWallets} active · {totals.reviewWallets} in review</span>}
        >
          <DataTable
            rows={wallets}
            columns={[
              { key: "label", header: "Wallet", render: (wallet) => (
                <div>
                  <p className="text-xs font-semibold text-[var(--a-fg)]">{wallet.label}</p>
                  <p className="text-[10px] text-[var(--a-muted)]">{wallet.provider ?? "Manual custody"}</p>
                </div>
              ) },
              { key: "asset", header: "Asset", render: (wallet) => <span className="a-mono text-xs font-bold text-[var(--a-accent)]">{wallet.asset}</span> },
              { key: "network", header: "Network", render: (wallet) => <span className="text-xs text-[var(--a-fg-2)]">{wallet.network}</span> },
              { key: "address", header: "Address", render: (wallet) => <span className="a-mono text-[10px] text-[var(--a-muted)]">{truncateAddress(wallet.address)}</span> },
              { key: "proof", header: "Proof file", render: (wallet) => <span className="text-xs text-[var(--a-fg-2)]">{wallet.fileName ?? "—"}</span> },
              { key: "status", header: "Status", render: (wallet) => <StatusChip value={wallet.status} /> },
              { key: "addedAt", header: "Added", render: (wallet) => <span className="text-xs text-[var(--a-muted)]">{fmtDateTime(wallet.addedAt)}</span> },
            ]}
          />
        </BentoCard>
      </div>
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <Label className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--a-muted)]">{label}</Label>
      {children}
...
function SecurityRow({ icon: Icon, label, value, good }: { icon: typeof ShieldCheck; label: string; value: string; good?: boolean }) {
  return (
